...
class PathFollower(Node):
    def __init__(self):
        ...
        # === 미션 일시정지 구독 (mission_manager 가 정지 지점에서 제어) ===
        # e-stop 노드가 /scout_mini/e_stop 을 센서 주기로 덮어쓰므로 별도 토픽 사용.
        self.mission_pause = False
        self.create_subscription(
            Bool,
            '/mission/pause',
            self.mission_pause_callback,
            10
        )
    ...
    # === 미션 일시정지 콜백 ===
    def mission_pause_callback(self, msg: Bool):
        if msg.data and not self.mission_pause:
            self.get_logger().info('미션 일시정지: 정지 지점에서 대기합니다.')
        elif not msg.data and self.mission_pause:
            self.get_logger().info('미션 재개: 주행을 계속합니다.')
        self.mission_pause = msg.data
    ...
    def timer_callback(self):
        # ★ 최상단에서 E-STOP / 미션 일시정지 우선 체크
        if self.estop_active or self.mission_pause:
            twist = Twist()
            twist.linear.x = 0.0
            twist.angular.z = 0.0
        ...
        try:
            ...
            self.get_logger().warn(
                f'TF ({self.global_frame} -> {self.base_frame}) 실패: [{type(e).__name__}] {e}'
            )
            return
